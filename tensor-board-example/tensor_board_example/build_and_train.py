    ...
    def nn_layer(input_tensor,
        ...
        with tf.name_scope(layer_name):
            with tf.name_scope('weights'):
                weights = initialize_weight_variable([input_dim, output_dim])
                variable_summaries(weights)
            with tf.name_scope('biases'):
                biases = initialize_bias_variable([output_dim])
    ...
    for i in range(FLAGS.max_steps):
        if i % 10 == 0:
            ...
            if i % 100 == 99:
                ...
                print('Adding run metadata for {} (of {})'.format(i, FLAGS.max_steps))
            else:
